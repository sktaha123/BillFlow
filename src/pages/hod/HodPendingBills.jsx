  ...
  useEffect(() => {
    const loadData = async () => {
      try {
        ...
        setBills(allBills.filter((b) => b.status === 'PENDING_HOD'));
        setAcademicYears(ay);
        setSemesters(sem);
      } catch (e) {
        console.error(e);
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto animate-in fade-in duration-200">
      ...
      <div className="flex items-center gap-2 text-xs font-medium text-slate-600">
        ...
        <span>{filteredBills.length} {filteredBills.length === 1 ? 'bill requires' : 'bills require'} your review</span>
      </div>
      ...
      {filteredBills.length === 0 ? (
        <EmptyState
          ...
          description={bills.length === 0 ? "Great! You have endorsed all pending bill claims." : "No pending bills match your filters."}
        />
      ) : (
                ...
                {filteredBills.map((b) => (
                  <tr key={b.id} className="hover:bg-slate-50/60 transition-colors">
                    ...
                    <td className="py-3.5 px-4 text-right">
                      <button
                        onClick={() => navigate(`/hod/review/${b.id}`)}
